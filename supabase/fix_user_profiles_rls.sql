...
-- Supprimer les anciennes policies pour recréer proprement
...
CREATE POLICY "Owner can insert user_profiles"
  ON user_profiles FOR INSERT
  WITH CHECK (auth.uid()::text = uid);

CREATE POLICY "Owner can update user_profiles"
  ON user_profiles FOR UPDATE
  USING (auth.uid()::text = uid);

CREATE POLICY "Owner can delete user_profiles"
