...
CREATE OR REPLACE FUNCTION _nom_profil(p_id UUID) RETURNS TEXT AS $$
  SELECT COALESCE(
...
SET titre = 'RDV avec ' || _nom_profil(r.pro_profile_id),
    updated_at = now()
FROM rdv r
WHERE ae.type = 'rdv'
  AND ae.rdv_id = r.id
  AND r.pro_profile_id IS NOT NULL
  AND _nom_profil(r.pro_profile_id) IS NOT NULL
  AND ae.titre IS DISTINCT FROM 'RDV avec ' || _nom_profil(r.pro_profile_id)
  AND ae.titre LIKE 'RDV avec %';

-- 2. Événements « côté pro » (couleur = 'rdv:<id>') : titre = « RDV avec <client> »
UPDATE agenda_events ae
SET titre = 'RDV avec ' || _nom_profil(r.client_profile_id),
    updated_at = now()
FROM rdv r
WHERE ae.type = 'rdv'
  ...
  AND _nom_profil(r.client_profile_id) IS NOT NULL
  AND ae.titre IS DISTINCT FROM 'RDV avec ' || _nom_profil(r.client_profile_id)
  AND ae.titre LIKE 'RDV avec %';

DROP FUNCTION _nom_profil(UUID);
