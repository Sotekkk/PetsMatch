// Génération du contrat de prestation de soins para-médicaux (ostéopathie /
// kinésithérapie animale) — HTML, signature électronique via
// /signer-contrat/[token] (même mécanisme que les contrats éleveur/garde/
// éducateur — voir contrat-garde.ts / contrat-education.ts). Miroir exact de
// contratSantePdfBytes côté appli (lib/pages/eleveur/animaux/contrat_pdf.dart).
...
export interface PrestationSante {
  animal_nom?: string;
  espece?: string;
  race?: string;
  client_nom?: string;
...
export interface DataContratSante {
  notes?: string;
}
...
h1 + p.subtitle{text-align:center;font-size:10.5px;color:#888;margin:0 0 8px}
...
export function generateContratSanteHTML(
  prestation: PrestationSante,
  praticien: PraticienInfo,
  data: DataContratSante,
): string {
  ...
  return `<!DOCTYPE html>
<html lang="fr"><head><meta charset="UTF-8"><title>Contrat de prestation de soins — ${prestation.animal_nom ?? ''}</title>
<style>${CSS}</style>
</head><body>
<div class="page">

<h1>Contrat de prestation de soins para-médicaux</h1>
<p class="subtitle">(ostéopathie / kinésithérapie animale)</p>

<div class="between">Entre les soussignés</div>

<div class="parties">
  <b>${praticien.nom}</b>, ci-après désigné « le Prestataire »<br>
  ${praticien.adresse ? praticien.adresse + '<br>' : ''}
  ${praticien.siret ? 'SIRET : ' + praticien.siret + '<br>' : ''}
  ${praticien.tel ? 'Tél : ' + praticien.tel + ' — ' : ''}${praticien.email}
</div>
...
<div class="art-title">Art. 1 – Nature de la prestation</div>
<div class="block">Le Prestataire intervient à titre de soin para-médical non conventionnel (ostéopathie et/ou kinésithérapie animale). Il ne se substitue pas à un acte vétérinaire et ne pose aucun diagnostic médical ni ne prescrit de traitement. Le Client est invité à consulter un vétérinaire en cas de suspicion de pathologie.</div>

<div class="art-title">Art. 2 – Comportement & sécurité de l'animal</div>
<div class="block">Le Client certifie que l'animal ne présente pas de contre-indication médicale connue aux soins pratiqués et déclare tout antécédent de morsure, d'attaque ou de comportement dangereux avant la première séance. Le Prestataire se réserve le droit d'interrompre une séance en cas de danger pour les personnes présentes.</div>
...
<div class="art-title">Art. 4 – Modalités financières & annulation</div>
...
<div class="art-title">Art. 5 – Résultats & obligation de moyens</div>
<div class="block">Le Prestataire s'engage à mettre en œuvre les moyens professionnels appropriés aux soins pratiqués. Les résultats dépendent également de l'état de l'animal et de facteurs qui lui sont propres ; le Prestataire est tenu à une obligation de moyens, non de résultat.</div>
...
<div class="sign-section">
  ...
  <div class="sign-row">
    ${signBlock('vendeur', 'Le Prestataire', praticien.nom)}
