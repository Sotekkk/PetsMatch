...
export const PAYS_LIST = ['France', 'Belgique', 'Suisse', 'Luxembourg'];

export const REGIONS_BY_PAYS: Record<string, string[]> = {
  France: [
...
export function departmentsInRegion(region: string): string[] {
...
export function fromPostalCode(cp: string): { departement: string; region: string } | null {
  if (!cp || cp.length < 5) return null;
  let key = cp.substring(0, 2);
  if (key === '97') key = cp.substring(0, 3);
  const info = DEPARTMENTS[key];
  if (!info) return null;
  return { departement: info.name, region: info.region };
}
