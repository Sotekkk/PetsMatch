...
export default function SignerContratPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data.type === 'contrat_hebergement') {
      ...
      } else if (data.type === 'contrat_prestation_toilettage') {
        const { data: rdv } = await supabase
          .from('rdv')
          .select('animal_id, client_uid, date_heure, adresse_depart, prestation_id')
          .eq('id', data.rdv_id)
        ...
        let animalInfo: { nom?: string; espece?: string } = {};
        let clientInfo: { nom?: string; contact?: string } = {};
        let prestationInfo: { nom?: string; prix_base?: number } = {};
        if (rdv?.animal_id) {
          const { data: an } = await supabase.from('animaux').select('nom, espece').eq('id', rdv.animal_id).maybeSingle();
          animalInfo = an ?? {};
        }
        ...
        if (rdv?.prestation_id) {
          const { data: pr } = await supabase.from('prestations_toilettage')
            .select('nom, prix_base').eq('id', rdv.prestation_id).maybeSingle();
          prestationInfo = pr ?? {};
        }
        generatedHtml = generateContratPrestationToilettageHTML(
          {
            client_nom: meta.client_nom || clientInfo.nom,
            client_contact: meta.client_contact || clientInfo.contact,
            animal_nom: animalInfo.nom ?? meta.acquereur_nom ?? '',
  ...
  async function signer(role: 'eleveur' | 'acquereur') {
    ...
    setSaving(role);
    try {
      const res = await fetch(`/api/contracts/${doc.id}/sign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          role, signature: dataUrl,
          actorUid: role === 'eleveur' ? user?.uid : undefined,
          actorNom: role === 'eleveur' ? (user?.displayName ?? '') : undefined,
        }),
      });
      const json = await res.json();
      if (!res.ok) { alert(json.error ?? 'Erreur lors de la signature'); setSaving(null); return; }

      const mergedMeta = json.metadata as Record<string, unknown>;
      const notBlank = (v: unknown) => v != null && String(v).trim() !== '';
      setDoc(prev => prev ? { ...prev, statut: json.statut, ...(json.signe_le ? { signe_le: json.signe_le } : {}), metadata: mergedMeta as DocRow['metadata'] } : prev);
  ...
  async function saveFemelle() {
    ...
    const res = await fetch(`/api/contracts/${doc.id}/femelle`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ femelleId: femelle.id, actorUid: user?.uid, actorEmail: user?.email }),
    });
    if (!res.ok) { const j = await res.json().catch(() => ({})); alert(j.error ?? 'Erreur'); setSavingFemelle(false); return; }
  ...
  async function refuser() {
    ...
    fetch('/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ uid: doc.uid_eleveur, type: 'contrat_refuse', title: '❌ Contrat refusé',
        ...
        data: { token } }) });
    // Le refus du devis d'éducation lié (metadata.devis_id) est géré côté
    // route API (api/contracts/[id]/refuse), qui a accès à jour au document.
  ...
  async function saveContactAcquereur() {
    ...
    const res = await fetch(`/api/contracts/${doc.id}/contact`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nom: contactNom, prenom: contactPrenom, adresse: contactAdresse, tel: contactTel }),
    });
    if (!res.ok) { const j = await res.json().catch(() => ({})); alert(j.error ?? 'Erreur'); setSavingContact(false); return; }
