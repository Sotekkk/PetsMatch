...
import { useProfessionPlanCode, PROFESSION_TOP_TIER } from '@/lib/use-plan';
...
export default function SanteContratPage() {
  const { user, userData, isSante, loading: authLoading } = useSanteAccess();
  const { planCode: santePlan } = useProfessionPlanCode('sante');
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase.from('documents_animaux').select('id, token, statut, animal_id, titre, created_at, metadata')
      .eq('uid_eleveur', user.uid).eq('type', 'contrat_sante');
  ...
  async function creerContrat(p: Paire) {
    ...
    try {
      const { data } = await supabase.from('documents_animaux').insert({
        ...
        type: 'contrat_sante',
        titre: `Contrat de prestation — ${p._client_nom}`,
        statut: 'brouillon',
        metadata: {
  ...
  const topTier = PROFESSION_TOP_TIER['sante'] ?? 'premium';
  if (santePlan !== topTier) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4 px-4 text-center">
        <span className="text-5xl">🔒</span>
        <h2 className="text-xl font-bold text-[#1F2A2E]" style={{ fontFamily: 'Galey, sans-serif' }}>
          Signature — Plan Pro requis
        </h2>
        <p className="text-gray-500 text-sm max-w-sm">
          La signature électronique de contrats est disponible avec un abonnement Pro.
        </p>
        <Link href="/sante/abonnement"
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <p className="text-sm text-gray-500 font-galey">
        Un contrat par client + animal, créé depuis un RDV confirmé.
      </p>
