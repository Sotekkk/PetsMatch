...
const PLAN_ICONS: Record<string, string> = { free: '🌱', essentiel: '⚡' };
...
function featureLabels(f: Record<string, boolean | number>): string[] {
  const out: string[] = [];
  out.push(f.maxPhotosPortfolio === -1 ? 'Portfolio illimité' : `${f.maxPhotosPortfolio ?? 5} photos portfolio`);
  if (f.hasMiseEnAvant) out.push('Mis en avant dans l\'annuaire');
  if (f.hasStatistiques) out.push('Statistiques de profil');
  return out;
}

function PhotographeAbonnementContent() {
  ...
  useEffect(() => {
    supabase.from('plans_tarifaires').select('*').eq('profil_type', 'photographe').eq('actif', true).order('prix_mensuel')
  ...
  useEffect(() => {
    ...
    const pending = sessionStorage.getItem('photographe_abonnement_success');
    if (pending) {
      setMessage({ type: 'success', text: pending });
      sessionStorage.removeItem('photographe_abonnement_success');
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const fetchPlan = async () => {
      const { data } = await supabase
        .from('abonnements').select('plan_code').eq('uid', user.uid).eq('profil_type', 'photographe').eq('statut', 'actif')
    ...
    const activate = async () => {
      if (sessionId) {
        try {
          ...
          if (json.ok && json.plan) {
            ...
            sessionStorage.setItem('photographe_abonnement_success', `🎉 Formule ${label} activée !`);
            router.replace('/photographe/abonnement');
            router.refresh();
            return;
          }
        } catch {
      ...
      const poll = async () => {
        ...
        if (plan !== 'free' || attempts >= 4) {
          ...
          sessionStorage.setItem('photographe_abonnement_success', plan !== 'free' ? `🎉 Formule ${label} activée !` : '✅ Paiement reçu, activation en cours…');
          router.replace('/photographe/abonnement');
          router.refresh();
          return;
        }
    ...
    try {
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({
          ...
          profil_type: 'photographe', returnPath: '/photographe/abonnement',
  ...
  const handlePortal = async () => {
    ...
    try {
      const res = await fetch('/api/stripe/portal', {
        ...
        body: JSON.stringify({ uid: user.uid, returnPath: '/photographe/abonnement' }),
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <h1 className="font-['Galey'] font-bold text-3xl text-[#1F2A2E] mb-2">Formules Photographe animalier</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10 max-w-2xl mx-auto">
        {plans.length === 0 ? (
          <div className="col-span-2 text-center text-gray-400 py-10">Chargement des formules…</div>
        ) : plans.map(plan => {
...
export default function PhotographeAbonnementPage() {
  return (
    <Suspense>
      <PhotographeAbonnementContent />
    </Suspense>
  );
}
