...
const PLAN_COLORS: Record<string, string> = {
  ...
  pro:     'border-[#7B5EA7] bg-white ring-2 ring-[#7B5EA7]/20',
...
function EducationAbonnementContent() {
  ...
  useEffect(() => {
    supabase.from('plans_tarifaires').select('*').eq('profil_type', 'education').eq('actif', true).order('prix_mensuel')
  ...
  useEffect(() => {
    ...
    const pending = sessionStorage.getItem('education_abonnement_success');
    if (pending) {
      setMessage({ type: 'success', text: pending });
      sessionStorage.removeItem('education_abonnement_success');
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const fetchPlan = async () => {
      const { data } = await supabase
        .from('abonnements').select('plan_code').eq('uid', user.uid).eq('profil_type', 'education').eq('statut', 'actif')
    ...
    const activate = async () => {
      if (sessionId) {
        try {
          ...
          if (json.ok && json.plan) {
            ...
            sessionStorage.setItem('education_abonnement_success', `🎉 Formule ${label} activée !`);
            router.replace('/education/abonnement');
            router.refresh();
            return;
          }
        } catch {
      ...
      const poll = async () => {
        ...
        if (plan !== 'free' || attempts >= 4) {
          ...
          sessionStorage.setItem('education_abonnement_success', plan !== 'free' ? `🎉 Formule ${label} activée !` : '✅ Paiement reçu, activation en cours…');
          router.replace('/education/abonnement');
          router.refresh();
          return;
        }
    ...
    try {
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({
          ...
          profil_type: 'education', returnPath: '/education/abonnement',
  ...
  const handlePortal = async () => {
    ...
    try {
      const res = await fetch('/api/stripe/portal', {
        ...
        body: JSON.stringify({ uid: user.uid, returnPath: '/education/abonnement' }),
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
  ...
  if (authLoading) return <div className="flex justify-center py-20"><div className="w-8 h-8 border-2 border-[#7B5EA7] border-t-transparent rounded-full animate-spin" /></div>;

  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <h1 className="font-['Galey'] font-bold text-3xl text-[#1F2A2E] mb-2">Formules Éducateur/Comportementaliste</h1>
        ...
        {plans.length === 0 ? (
          ...
          return (
            <div key={plan.plan_code} className={`rounded-2xl border p-6 flex flex-col relative ${PLAN_COLORS[plan.plan_code] ?? 'border-gray-200 bg-white'}`}>
              {plan.plan_code === 'pro' && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#7B5EA7] text-white text-xs font-bold px-3 py-0.5 rounded-full">
                  Populaire
                </div>
              )}
              ...
              {isCurrent ? (
                <div className="text-center text-sm font-semibold text-[#7B5EA7] bg-[#F3EFF8] py-2 rounded-xl">
                  Formule actuelle
                </div>
              ) : plan.plan_code === 'free' ? (
                ...
                <button onClick={() => handleSubscribe(plan.plan_code)}
                  ...
                  className="w-full py-2.5 rounded-xl text-sm font-semibold bg-[#7B5EA7] text-white hover:bg-[#684F8F] disabled:opacity-50 transition-colors">
      ...
      {currentPlan !== 'free' && user && (
        <div className="text-center mb-4">
          <button onClick={handlePortal} disabled={loadingPortal}
            className="text-sm text-gray-500 hover:text-[#7B5EA7] underline">
      ...
      <p className="text-center text-xs text-gray-400">
        Une question sur les formules éducateur/comportementaliste ? Contactez-nous depuis votre espace pro.
      </p>
    </div>
  );
}

export default function EducationAbonnementPage() {
  return (
    <Suspense>
      <EducationAbonnementContent />
    </Suspense>
  );
}
