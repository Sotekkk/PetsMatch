...
const PLAN_COLORS: Record<string, string> = {
  ...
  pro:     'border-[#FFB74D] bg-white ring-2 ring-[#FFB74D]/20',
  premium: 'border-[#D97706] bg-white ring-2 ring-[#D97706]/20',
};

function featureLabels(f: Record<string, boolean | number>): string[] {
  const out: string[] = [];
  out.push(f.hasEmployesIllimites ? 'Employés illimités' : `Jusqu'à ${f.maxEmployes} employé`);
...
function ToilettageAbonnementContent() {
  ...
  useEffect(() => {
    supabase.from('plans_tarifaires').select('*').eq('profil_type', 'toilettage').eq('actif', true).order('prix_mensuel')
  ...
  useEffect(() => {
    ...
    const pending = sessionStorage.getItem('toilettage_abonnement_success');
    if (pending) {
      setMessage({ type: 'success', text: pending });
      sessionStorage.removeItem('toilettage_abonnement_success');
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const fetchPlan = async () => {
      const { data } = await supabase
        .from('abonnements').select('plan_code').eq('uid', user.uid).eq('profil_type', 'toilettage').eq('statut', 'actif')
    ...
    const activate = async () => {
      if (sessionId) {
        try {
          ...
          if (json.ok && json.plan) {
            ...
            sessionStorage.setItem('toilettage_abonnement_success', `🎉 Formule ${label} activée !`);
            router.replace('/toilettage/abonnement');
            router.refresh();
            return;
          }
        } catch {
      ...
      const poll = async () => {
        ...
        if (plan !== 'free' || attempts >= 4) {
          ...
          sessionStorage.setItem('toilettage_abonnement_success', plan !== 'free' ? `🎉 Formule ${label} activée !` : '✅ Paiement reçu, activation en cours…');
          router.replace('/toilettage/abonnement');
          router.refresh();
          return;
        }
    ...
    try {
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({
          ...
          profil_type: 'toilettage', returnPath: '/toilettage/abonnement',
  ...
  const handlePortal = async () => {
    ...
    try {
      const res = await fetch('/api/stripe/portal', {
        ...
        body: JSON.stringify({ uid: user.uid, returnPath: '/toilettage/abonnement' }),
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <h1 className="font-['Galey'] font-bold text-3xl text-[#1F2A2E] mb-2">Formules Toiletteur</h1>
        ...
        {plans.length === 0 ? (
          ...
          return (
            <div key={plan.plan_code} className={`rounded-2xl border p-6 flex flex-col relative ${PLAN_COLORS[plan.plan_code] ?? 'border-gray-200 bg-white'}`}>
              {plan.plan_code === 'pro' && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#FFB74D] text-white text-xs font-bold px-3 py-0.5 rounded-full">
                  Populaire
                </div>
              )}
...
export default function ToilettageAbonnementPage() {
  return (
    <Suspense>
      <ToilettageAbonnementContent />
    </Suspense>
  );
}
