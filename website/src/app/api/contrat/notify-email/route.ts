...
export async function POST(req: NextRequest) {
  const { email, client_nom, pro_nom, titre, signing_url } =
...
<body style="margin:0;padding:0;background:#f5f7fa;font-family:'Segoe UI',Arial,sans-serif;">
  <div style="max-width:580px;margin:32px auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px;">
      ...
      <p style="font-size:14px;color:#4B5563;line-height:1.6;margin:0 0 24px;">
        <strong>${pro_nom}</strong> vous envoie ${titre ? `« ${titre} »` : 'un contrat'} à signer.<br/>
        Veuillez signer le contrat en cliquant sur le bouton ci-dessous.
      </p>
      ...
      <p style="font-size:12px;color:#9CA3AF;text-align:center;margin:0;">
        Lien valable tant que le contrat est en attente.<br/>
  ...
  try {
    await mailTransporter.sendMail({
      from: MAIL_FROM,
      to: email,
      subject: `✍️ Signature requise — ${pro_nom} · PetsMatch`,
      html,
    });
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('[contrat/notify-email]', err);
