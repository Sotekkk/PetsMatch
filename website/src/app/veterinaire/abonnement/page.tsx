...
const PLAN_ICONS: Record<string, string> = { free: '🌱', avance: '⚡', clinique: '👑' };
const PLAN_COLORS: Record<string, string> = {
  free:     'border-gray-200 bg-white',
  avance:   'border-[#0C5C6C] bg-white ring-2 ring-[#0C5C6C]/20',
...
function VeterinaireAbonnementContent() {
  ...
  useEffect(() => {
    supabase.from('plans_tarifaires').select('*').eq('profil_type', 'veterinaire').eq('actif', true).order('prix_mensuel')
  ...
  useEffect(() => {
    ...
    const pending = sessionStorage.getItem('veterinaire_abonnement_success');
    if (pending) {
      setMessage({ type: 'success', text: pending });
      sessionStorage.removeItem('veterinaire_abonnement_success');
    }
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const fetchPlan = async () => {
      const { data } = await supabase
        .from('abonnements').select('plan_code').eq('uid', user.uid).eq('profil_type', 'veterinaire').eq('statut', 'actif')
    ...
    const activate = async () => {
      if (sessionId) {
        try {
          ...
          if (json.ok && json.plan) {
            ...
            sessionStorage.setItem('veterinaire_abonnement_success', `🎉 Formule ${label} activée !`);
            router.replace('/veterinaire/abonnement');
            router.refresh();
            return;
          }
        } catch {
      ...
      const poll = async () => {
        ...
        if (plan !== 'free' || attempts >= 4) {
          ...
          sessionStorage.setItem('veterinaire_abonnement_success', plan !== 'free' ? `🎉 Formule ${label} activée !` : '✅ Paiement reçu, activation en cours…');
          router.replace('/veterinaire/abonnement');
          router.refresh();
          return;
        }
    ...
    try {
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({
          ...
          profil_type: 'veterinaire', returnPath: '/veterinaire/abonnement',
  ...
  const handlePortal = async () => {
    ...
    try {
      const res = await fetch('/api/stripe/portal', {
        ...
        body: JSON.stringify({ uid: user.uid, returnPath: '/veterinaire/abonnement' }),
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <h1 className="font-['Galey'] font-bold text-3xl text-[#1F2A2E] mb-2">Formules Vétérinaire</h1>
...
export default function VeterinaireAbonnementPage() {
  return (
    <Suspense>
      <VeterinaireAbonnementContent />
    </Suspense>
  );
}
