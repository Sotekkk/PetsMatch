...
function AnimalCard({ a, tab, showPorteeBadge = false, onDelete }: {
  a: Animal; tab: 'presents' | 'anciens'; showPorteeBadge?: boolean; onDelete?: () => void;
}) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {fetching ? (
            ...
            return (
              <div key={pid}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {members.map(a => <AnimalCard key={a.id} a={a} tab={tab} showPorteeBadge onDelete={() => deleteAnimal(a.id)} />)}
                </div>
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {currentList.map(a => <AnimalCard key={a.id} a={a} tab={tab} onDelete={() => deleteAnimal(a.id)} />)}
        </div>
      )}
