...
// ── Contrats de prestation santé (ostéopathie / kinésithérapie) ───────────────
// Un contrat par CLIENT + ANIMAL (pas par séance). Réutilise `documents_animaux`
// (type='contrat_sante') + ContratSignaturePage / /signer-contrat — même
// mécanisme que education_contrats_page.dart.

class SanteContratsPage extends StatefulWidget {
  const SanteContratsPage({super.key});

  @override
  State<SanteContratsPage> createState() => _SanteContratsPageState();
}

class _SanteContratsPageState extends State<SanteContratsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      var q = _supa.from('documents_animaux').select().eq('uid_eleveur', uid).eq('type', 'contrat_sante');
        ...
        body: Center(
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 12),
              const Text('Signature — Plan Pro requis',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontFamily: 'Galey', fontWeight: FontWeight.w700, fontSize: 16)),
              const SizedBox(height: 8),
              Text('La signature électronique de contrats est disponible avec l\'abonnement Pro.',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontFamily: 'Galey', fontSize: 13, color: Colors.grey.shade500)),
              const SizedBox(height: 20),
              ElevatedButton(
                onPressed: () => Navigator.push(context,
                    MaterialPageRoute(builder: (_) => const SanteAbonnementPage(profilType: 'sante'))),
              ...
              ? Center(
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const Padding(
                      ...
                      child: Text(
                        'Créez un contrat pour un client + son animal depuis un RDV confirmé.',
                        textAlign: TextAlign.center,
