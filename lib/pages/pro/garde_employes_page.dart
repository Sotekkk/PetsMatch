...
const kJoursSemaineGarde = ['lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi', 'dimanche'];
const kCouleursPlanningGarde = ['#6E9E57', '#FFB74D', '#4DB6AC', '#7986CB', '#F06292', '#BA68C8'];

/// Types de prestations garde/pet-sitting — mêmes motifs que ceux proposés à
/// la réservation (rdv_booking_page.dart), utilisés ici comme "compétences"
/// pour restreindre les missions confiées à un employé (ex. quelqu'un qui ne
/// fait que des promenades, pas de garde à domicile).
const kTypesServiceGarde = <String, String>{
...
/// Employés enrichis garde (formule pro+, hasEmployes) — couleur planning,
/// compétences (types de prestations confiées), horaires, congés. Vient en
/// complément de EmployesPage (invitation/permissions générique) plutôt que
/// la remplacer — même architecture que ToilettageEmployesPage/PensionEmployesPage.
class GardeEmployesPage extends StatefulWidget {
  const GardeEmployesPage({super.key});

  @override
  State<GardeEmployesPage> createState() => _GardeEmployesPageState();
}

class _GardeEmployesPageState extends State<GardeEmployesPage> {
  static const _green = Color(0xFF6E9E57);
  static const _bg = Color(0xFFF8F8F6);
  final _supa = Supabase.instance.client;

  bool _loading = true;
  bool _hasEmployes = false;
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        PlanService.getGardePlanCode(uid),
        // .neq('type', 'benevole') excluait TOUT le monde : ce champ n'est
        // jamais renseigné pour un employé normal (seuls les bénévoles
        // association l'ont), et NULL != 'benevole' n'est jamais vrai en
        // SQL — donc les lignes NULL étaient exclues (bug constaté et
        // corrigé côté toilettage, appliqué ici direct).
        pid.isNotEmpty
      ...
      if (mounted) {
        setState(() {
          _hasEmployes = PlanService.getGardeConfig(planCode).hasEmployes;
  ...
  Future<void> _openEdit(Map<String, dynamic> employe) async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (_) => _EmployeGardeForm(employe: employe),
    );
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const EmployesPage(profileType: 'garde'))),
          ),
        ],
      ),
      body: _loading
          ...
          : !_hasEmployes
              ? Center(
                  child: Padding(
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      const Text('Fonctionnalité réservée aux formules Pro et Premium',
                          textAlign: TextAlign.center, style: TextStyle(fontFamily: 'Galey', fontWeight: FontWeight.w600, fontSize: 15)),
                      const SizedBox(height: 6),
                      Text('Couleur planning, prestations confiées, horaires et congés par employé.',
                          textAlign: TextAlign.center, style: TextStyle(fontFamily: 'Galey', fontSize: 13, color: Colors.grey.shade600)),
                      const SizedBox(height: 16),
                      ElevatedButton(
                        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const GardeAbonnementPage())),
              ...
              : _employes.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        final e = _employes[i];
                        final couleur = Color(int.parse((e['couleur_planning'] as String? ?? '#6E9E57').replaceFirst('#', '0xFF')));
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(competences.isEmpty
                                ? 'Toutes prestations'
                                : competences.map((c) => kTypesServiceGarde[c] ?? c).join(', '),
...
class _EmployeGardeForm extends StatefulWidget {
  final Map<String, dynamic> employe;
  const _EmployeGardeForm({required this.employe});

  @override
  State<_EmployeGardeForm> createState() => _EmployeGardeFormState();
}

class _EmployeGardeFormState extends State<_EmployeGardeForm> {
  static const _green = Color(0xFF6E9E57);
  final _supa = Supabase.instance.client;
  ...
  void initState() {
    super.initState();
    _couleur = widget.employe['couleur_planning'] as String? ?? kCouleursPlanningGarde.first;
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 8, children: kCouleursPlanningGarde.map((c) {
          ...
          const SizedBox(height: 16),
          Text('Prestations confiées (vide = toutes)', style: TextStyle(fontFamily: 'Galey', fontSize: 12, color: Colors.grey.shade600)),
          const SizedBox(height: 8),
          Wrap(spacing: 6, runSpacing: 6, children: kTypesServiceGarde.entries.map((entry) {
          ...
          const SizedBox(height: 8),
          Wrap(spacing: 6, runSpacing: 6, children: kJoursSemaineGarde.map((j) {
