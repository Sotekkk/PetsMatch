...
class _FichesPensionPageState extends State<FichesPensionPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // Profils proprio depuis Firestore (nom + téléphone)
      final ownerUids = (acces as List)
          .map((a) => a['owner_uid'] as String? ?? '')
          .where((u) => u.isNotEmpty)
          .toSet();
      final ownerProfiles = <String, Map<String, String>>{};
      for (final uid in ownerUids) {
        try {
          final doc = await FirebaseFirestore.instance.collection('users').doc(uid).get();
          final d = doc.data();
          if (d != null) {
            final prenom = d['firstname'] as String? ?? '';
            final nom    = d['lastname'] as String? ?? '';
            final tel    = d['phone_number'] as String? ?? '';
            final mail = d['email'] as String? ?? '';
            ownerProfiles[uid] = {
              'nom':     [prenom, nom].where((s) => s.isNotEmpty).join(' '),
              'contact': tel,
              'email':   mail,
            };
          }
      ...
      final items = (acces as List).map((a) {
        final animal  = animalMap[a['animal_id'] as String] ?? {};
        final ownerUid = a['owner_uid'] as String? ?? '';
        final owner   = ownerProfiles[ownerUid] ?? {};
        return _FicheItem(
