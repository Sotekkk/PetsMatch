...
class GardeAbonnementPage extends StatefulWidget {
  const GardeAbonnementPage({super.key});

  @override
  State<GardeAbonnementPage> createState() => _GardeAbonnementPageState();
}

class _GardeAbonnementPageState extends State<GardeAbonnementPage> {
  static const _teal  = Color(0xFF0C5C6C);
  static const _bg    = Color(0xFFF8F8F6);

  String _planCode = 'free';
  Map<String, GardePlanConfig> _plans = PlanService.gardeConfigs;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final results = await Future.wait([
      ...
      PlanService.getGardePlansLive(),
    ]);
    if (!mounted) return;
    setState(() {
      _planCode = results[0] as String;
      _plans    = results[1] as Map<String, GardePlanConfig>;
      _loading  = false;
    });
  }

  Future<void> _openWebsite([String path = '/garde/abonnement']) async {
  ...
  Widget build(BuildContext context) {
    final config = _plans[_planCode] ?? PlanService.getGardeConfig(_planCode);
          ...
          : ListView(
              ...
              children: [
                ...
                for (final code in ['free', 'pro', 'premium']) ...[
                  _GardePlanCard(
                    config: _plans[code] ?? PlanService.getGardeConfig(code),
                    isCurrent: _planCode == code,
                    onSelect: _planCode == code ? null : () => _openWebsite('/garde/abonnement'),
                  ),
                  const SizedBox(height: 10),
                ],
                const SizedBox(height: 10),
                TextButton(
                  onPressed: () => _openWebsite('/garde/abonnement'),
  ...
  String _summaryLine(GardePlanConfig c) {
    final parts = <String>[
...
class _GardePlanCard extends StatelessWidget {
  final GardePlanConfig config;
  final bool isCurrent;
  final VoidCallback? onSelect;

  const _GardePlanCard({required this.config, required this.isCurrent, required this.onSelect});
