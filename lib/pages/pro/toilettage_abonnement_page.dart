...
/// Formules Toiletteur — Gratuit/Pro/Premium, sur le modèle de
/// sante_abonnement_page.dart. Grille dédiée (pas partagée avec un autre
/// profil_type).
class ToilettageAbonnementPage extends StatefulWidget {
  const ToilettageAbonnementPage({super.key});

  @override
  State<ToilettageAbonnementPage> createState() => _ToilettageAbonnementPageState();
}

class _ToilettageAbonnementPageState extends State<ToilettageAbonnementPage> {
  static const _orange = Color(0xFFFFB74D);
  static const _bg = Color(0xFFF8F8F6);

  String _planCode = 'free';
  Map<String, ToilettagePlanConfig> _plans = PlanService.toilettageConfigs;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final results = await Future.wait([
      ...
      PlanService.getToilettagePlansLive(),
    ]);
    if (!mounted) return;
    setState(() {
      _planCode = results[0] as String;
      _plans    = results[1] as Map<String, ToilettagePlanConfig>;
      _loading  = false;
    });
  }

  Future<void> _openWebsite() async {
    final uri = Uri.parse('${PlanService.kWebsiteUrl}/toilettage/abonnement');
  ...
  Widget build(BuildContext context) {
    final config = _plans[_planCode] ?? PlanService.getToilettageConfig(_planCode);
          ...
          : ListView(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      colors: [Color(0xFFFFB74D), Color(0xFFFFD08A)],
                ...
                for (final code in ['free', 'pro', 'premium']) ...[
                  _ToilettagePlanCard(
                    config: _plans[code] ?? PlanService.getToilettageConfig(code),
                ...
                TextButton(
                  ...
                  child: const Text('Gérer mon abonnement sur le site →',
                      style: TextStyle(fontFamily: 'Galey', color: _orange)),
                ),
  ...
  String _summaryLine(ToilettagePlanConfig c) {
    final parts = <String>[
      c.hasEmployesIllimites ? 'Employés illimités' : '${c.maxEmployes} employé',
      if (c.hasFacturation) 'Facturation',
      if (c.hasPlanningEmployes) 'Planning employés',
    ];
    return parts.join(' · ');
  }
}

class _ToilettagePlanCard extends StatelessWidget {
  final ToilettagePlanConfig config;
  final bool isCurrent;
  final VoidCallback? onSelect;

  const _ToilettagePlanCard({required this.config, required this.isCurrent, required this.onSelect});

  @override
  Widget build(BuildContext context) {
    const orange = Color(0xFFFFB74D);
    final accent = config.code == 'premium'
        ? const Color(0xFFD97706)
        : config.code == 'pro' ? orange : const Color(0xFF6B7280);
    final badge = config.code == 'premium' ? '👑' : config.code == 'pro' ? '⚡' : '🌱';

    final features = <String?>[
      config.hasEmployesIllimites ? 'Employés illimités' : 'Jusqu\'à ${config.maxEmployes} employé',
      config.hasFacturation ? 'Facturation' : null,
      config.hasStatistiques ? 'Statistiques' : null,
      config.hasGalerie ? 'Galerie' : null,
      config.hasNotifications ? 'Notifications' : null,
