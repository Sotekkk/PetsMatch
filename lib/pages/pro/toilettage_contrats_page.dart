...
// ── Contrats de prestation toilettage ─────────────────────────────────────
// Un contrat par RDV (type='contrat_prestation_toilettage'), généré depuis la
// carte RDV (pro_agenda.dart → _genererContratToilettage). Cette page ne
// fait que lister/rouvrir ce qui existe déjà — pas de création manuelle ici.

class ToilettageContratsPage extends StatefulWidget {
  const ToilettageContratsPage({super.key});

  @override
  State<ToilettageContratsPage> createState() => _ToilettageContratsPageState();
}

class _ToilettageContratsPageState extends State<ToilettageContratsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      var q = _supa.from('documents_animaux').select().eq('uid_eleveur', uid).eq('type', 'contrat_prestation_toilettage');
