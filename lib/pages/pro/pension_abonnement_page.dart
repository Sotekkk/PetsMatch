...
class PensionAbonnementPage extends StatefulWidget {
  const PensionAbonnementPage({super.key});

  @override
  State<PensionAbonnementPage> createState() => _PensionAbonnementPageState();
}

class _PensionAbonnementPageState extends State<PensionAbonnementPage> {
  static const _teal  = Color(0xFF0C5C6C);
  static const _bg    = Color(0xFFF8F8F6);

  String _planCode = 'free';
  Map<String, PensionPlanConfig> _plans = PlanService.pensionConfigs;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final results = await Future.wait([
      ...
      PlanService.getPensionPlansLive(),
    ]);
    if (!mounted) return;
    setState(() {
      _planCode = results[0] as String;
      _plans    = results[1] as Map<String, PensionPlanConfig>;
      _loading  = false;
    });
  }

  Future<void> _openWebsite([String path = '/pension/abonnement']) async {
  ...
  Widget build(BuildContext context) {
    final config = _plans[_planCode] ?? PlanService.getPensionConfig(_planCode);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Mon abonnement pension',
          ...
          : ListView(
              ...
              children: [
                ...
                for (final code in ['free', 'pro', 'premium']) ...[
                  _PensionPlanCard(
                    config: _plans[code] ?? PlanService.getPensionConfig(code),
                    isCurrent: _planCode == code,
                    onSelect: _planCode == code ? null : () => _openWebsite('/pension/abonnement'),
                  ),
                  const SizedBox(height: 10),
                ],
                const SizedBox(height: 10),
                TextButton(
                  onPressed: () => _openWebsite('/pension/abonnement'),
  ...
  String _summaryLine(PensionPlanConfig c) {
    final parts = <String>[
      c.logementsIllimites ? 'Logements illimités' : '1 logement',
...
class _PensionPlanCard extends StatelessWidget {
  final PensionPlanConfig config;
  final bool isCurrent;
  final VoidCallback? onSelect;

  const _PensionPlanCard({required this.config, required this.isCurrent, required this.onSelect});
  ...
  Widget build(BuildContext context) {
    ...
    final features = <String?>[
      config.logementsIllimites ? 'Logements illimités' : '1 logement',
