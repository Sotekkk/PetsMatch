...
/// Formules vétérinaire (Spec §8.1) — Découverte / Avancé / Clinique.
class VetAbonnementPage extends StatefulWidget {
  const VetAbonnementPage({super.key});

  @override
  State<VetAbonnementPage> createState() => _VetAbonnementPageState();
}

class _VetAbonnementPageState extends State<VetAbonnementPage> {
  static const _teal = Color(0xFF0C5C6C);
  static const _bg   = Color(0xFFF8F8F6);

  String _planCode = 'free';
  Map<String, VetPlanConfig> _plans = PlanService.vetConfigs;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final results = await Future.wait([
      PlanService.getVetPlanCode(uid),
      PlanService.getVetPlansLive(),
    ]);
    if (!mounted) return;
    setState(() {
      _planCode = results[0] as String;
      _plans    = results[1] as Map<String, VetPlanConfig>;
      _loading  = false;
    });
  }

  Future<void> _openWebsite() async {
    final uri = Uri.parse('${PlanService.kWebsiteUrl}/veterinaire/abonnement');
  ...
  Widget build(BuildContext context) {
    final config = _plans[_planCode] ?? PlanService.getVetConfig(_planCode);
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                for (final code in ['free', 'avance', 'clinique']) ...[
                  _VetPlanCard(
                    config: _plans[code] ?? PlanService.getVetConfig(code),
  ...
  String _summaryLine(VetPlanConfig c) {
    final parts = <String>[
...
class _VetPlanCard extends StatelessWidget {
  final VetPlanConfig config;
  final bool isCurrent;
  final VoidCallback? onSelect;

  const _VetPlanCard({required this.config, required this.isCurrent, required this.onSelect});

  @override
  Widget build(BuildContext context) {
    const teal = Color(0xFF0C5C6C);
    final accent = config.code == 'clinique'
        ? const Color(0xFFD97706)
        : config.code == 'avance' ? teal : const Color(0xFF6B7280);
    final badge = config.code == 'clinique' ? '👑' : config.code == 'avance' ? '⚡' : '🌱';

    final features = <String?>[
      config.hasAccesPermanent ? 'Accès lecture permanent' : 'Annuaire basique, token 72h',
      config.hasEcritureCarnetSante ? 'Écriture carnet santé' : null,
      config.hasRappelsPush ? 'Rappels push' : null,
