...
/// Employés enrichis pension (formule pro+, hasEmployes) — couleur planning,
/// compétences (espèces prises en charge), horaires, congés. Vient en
/// complément de EmployesPage (invitation/permissions générique) plutôt que
/// la remplacer — même architecture que ToilettageEmployesPage.
class PensionEmployesPage extends StatefulWidget {
  const PensionEmployesPage({super.key});

  @override
  State<PensionEmployesPage> createState() => _PensionEmployesPageState();
}

class _PensionEmployesPageState extends State<PensionEmployesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        PlanService.getPensionPlanCode(uid),
        pid.isNotEmpty
            ...
            // (bug constaté et corrigé côté toilettage, appliqué ici direct).
      ...
      if (mounted) setState(() {
        _hasEmployes = PlanService.getPensionConfig(planCode).hasEmployes;
  ...
  Future<void> _openEdit(Map<String, dynamic> employe) async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (_) => _EmployePensionForm(employe: employe),
    );
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const EmployesPage(profileType: 'pension'))),
          ),
        ],
      ),
      body: _loading
          ...
          : !_hasEmployes
              ? Center(
                  child: Padding(
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text('Couleur planning, espèces prises en charge, horaires et congés par employé.',
                          textAlign: TextAlign.center, style: TextStyle(fontFamily: 'Galey', fontSize: 13, color: Colors.grey.shade600)),
                      const SizedBox(height: 16),
                      ElevatedButton(
                        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PensionAbonnementPage())),
              ...
              : _employes.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        final e = _employes[i];
                        final couleur = Color(int.parse((e['couleur_planning'] as String? ?? '#4DB6AC').replaceFirst('#', '0xFF')));
...
class _EmployePensionForm extends StatefulWidget {
  final Map<String, dynamic> employe;
  const _EmployePensionForm({required this.employe});

  @override
  State<_EmployePensionForm> createState() => _EmployePensionFormState();
}

class _EmployePensionFormState extends State<_EmployePensionForm> {
  static const _teal = Color(0xFF0C5C6C);
  final _supa = Supabase.instance.client;
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 8, children: kCouleursPlanningPension.map((c) {
          ...
          const SizedBox(height: 16),
          Text('Espèces prises en charge (vide = toutes)', style: TextStyle(fontFamily: 'Galey', fontSize: 12, color: Colors.grey.shade600)),
          const SizedBox(height: 8),
          Wrap(spacing: 6, runSpacing: 6, children: kPensionEspeces.map((e) {
            final key = e['key']!;
            final selected = _competences.contains(key);
            return FilterChip(
              label: Text('${e['emoji'] ?? ''} ${e['label']}', style: const TextStyle(fontFamily: 'Galey', fontSize: 12)),
              selected: selected,
              selectedColor: _teal.withValues(alpha: 0.15),
              onSelected: (v) => setState(() => v ? _competences.add(key) : _competences.remove(key)),
            );
          }).toList()),
          const SizedBox(height: 16),
          Text('Jours travaillés', style: TextStyle(fontFamily: 'Galey', fontSize: 12, color: Colors.grey.shade600)),
          const SizedBox(height: 8),
          Wrap(spacing: 6, runSpacing: 6, children: kJoursSemainePension.map((j) {
