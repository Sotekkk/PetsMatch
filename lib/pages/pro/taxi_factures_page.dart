...
/// Taxi animalier — historique des factures, sur le modèle de
/// pension_factures_page.dart.
class TaxiFacturesPage extends StatefulWidget {
  const TaxiFacturesPage({super.key});

  @override
  State<TaxiFacturesPage> createState() => _TaxiFacturesPageState();
}

class _TaxiFacturesPageState extends State<TaxiFacturesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final rows = await _supa.from('taxi_factures').select()
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        pw.TableHelper.fromTextArray(
          ...
          headerDecoration: const pw.BoxDecoration(color: PdfColor.fromInt(0xFF00838F)),
  ...
  Future<void> _marquerPayee(String id) async {
    await _supa.from('taxi_factures').update({
      'statut': 'payee',
