...
// ── Contrats de prestation photo ──────────────────────────────────────────
// Un contrat par RDV (type='contrat_prestation_photo'), généré depuis la
// carte RDV (pro_agenda.dart → _genererContratPhoto). Cette page ne fait que
// lister/rouvrir ce qui existe déjà — pas de création manuelle ici.

class PhotographeContratsPage extends StatefulWidget {
  const PhotographeContratsPage({super.key});

  @override
  State<PhotographeContratsPage> createState() => _PhotographeContratsPageState();
}

class _PhotographeContratsPageState extends State<PhotographeContratsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      var q = _supa.from('documents_animaux').select().eq('uid_eleveur', uid).eq('type', 'contrat_prestation_photo');
