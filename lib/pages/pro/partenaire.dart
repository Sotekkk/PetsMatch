...
class User {
  ...
  factory User.fromMap(Map<String, dynamic> data, String documentId) {
    return User(
      uid: documentId,
      nameElevage: data['nameElevage'] ?? '',
      profilePictureUrlElevage: data['profilePictureUrlElevage'] ?? '',
      descEntreprise: data['descEntreprise'] ?? '',
...
class FirebaseService {
  ...
  Future<List<User>> getUsers() async {
    QuerySnapshot querySnapshot = await _firestore
        .collection('users')
        .where('catPro', isEqualTo: 'Prestataire')
        .get();
...
class PrestatairePage extends StatefulWidget {
  @override
  _PrestatairePageState createState() => _PrestatairePageState();
}
