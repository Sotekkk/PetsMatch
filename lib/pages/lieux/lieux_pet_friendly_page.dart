...
class _LieuxPetFriendlyPageState extends State<LieuxPetFriendlyPage> {
  ...
  // likes locaux (uid → set de place_id)
  ...
  Future<void> _load({bool reset = false}) async {
    if (reset) {
      setState(() { _loading = true; _page = 0; _lieux = []; _hasMore = true; });
    } else {
    ...
    try {
      ...
      var base = _supabase
          ...
          .select('id, uid_pro, nom, categorie, sous_categorie, ville, lat, lng, especes_acceptees, horaires, photo_profil_url, photos, note_moyenne, nb_avis, nb_likes, plan, statut')
          .eq('statut', 'actif');

      if (_categorie != 'tous') base = base.eq('categorie', _categorie);
      if (_espece != 'tous') base = base.contains('especes_acceptees', [_espece]);
      ...
      final data = await (_sortBy == 'note'
              ...
              : _sortBy == 'misEnAvant'
                  ? base.order('plan', ascending: false).order('created_at', ascending: false)
                  : base.order('created_at', ascending: false))
          .range(from, from + _pageSize - 1);
      final rows = List<Map<String, dynamic>>.from(data as List);

      // Charger les likes de l'utilisateur
      if (_uid != null && rows.isNotEmpty) {
        ...
        for (final l in liked as List) {
          _likedIds.add(l['place_id'] as String);
        }
      }

      for (final r in rows) {
        _likeCounts[r['id'] as String] = (r['nb_likes'] as int? ?? 0);
      }
  ...
  Future<void> _toggleLike(String placeId) async {
    if (_uid == null) return;
    final isLiked = _likedIds.contains(placeId);
    setState(() {
      if (isLiked) {
        _likedIds.remove(placeId);
        _likeCounts[placeId] = (_likeCounts[placeId] ?? 1) - 1;
      } else {
        _likedIds.add(placeId);
        _likeCounts[placeId] = (_likeCounts[placeId] ?? 0) + 1;
      }
    });
    try {
      if (isLiked) {
        await _supabase.from('place_likes')
            .delete()
            .eq('place_id', placeId)
            .eq('user_uid', _uid);
        await _supabase.from('petfriendly_places')
            .update({'nb_likes': (_likeCounts[placeId] ?? 0)})
            .eq('id', placeId);
      } else {
        await _supabase.from('place_likes')
            .insert({'place_id': placeId, 'user_uid': _uid});
        await _supabase.from('petfriendly_places')
            .update({'nb_likes': (_likeCounts[placeId] ?? 0)})
            .eq('id', placeId);
      }
      ...
      setState(() {
        if (isLiked) {
          ...
          _likedIds.remove(placeId);
          _likeCounts[placeId] = max(0, (_likeCounts[placeId] ?? 1) - 1);
        }
      });
    }
  }
      ...
      child: Column(
        ...
        children: [
          SingleChildScrollView(
            ...
            child: Row(children: [
              _Chip('Tout',          'tous',          categorie, onCategorie, Icons.explore_outlined),
              const SizedBox(width: 8),
              _Chip('Hébergements',  'hebergement',   categorie, onCategorie, Icons.hotel_outlined),
              const SizedBox(width: 8),
              _Chip('Cafés & Restos','restauration',  categorie, onCategorie, Icons.restaurant_outlined),
            ]),
          ),
          ...
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(children: [
              _Chip('Toutes espèces','tous',    espece, onEspece, Icons.pets_outlined),
              const SizedBox(width: 8),
...
class _LieuCard extends StatelessWidget {
  ...
  String _ouvertLabel(Map<String, dynamic> lieu) {
    ...
    if (val == null || val.toLowerCase() == 'fermé' || val.toLowerCase() == 'ferme') return '🔴 Fermé';
  ...
  String _especeEmoji(String e) {
    const map = {'chien':'🐶','chat':'🐱','cheval':'🐴','lapin':'🐰','oiseau':'🦜','nac':'🐾'};
                ...
                children: [
                  photoUrl.isNotEmpty
                      ? CachedNetworkImage(
                          imageUrl: photoUrl,
                          fit: BoxFit.cover,
                          errorWidget: (_, __, ___) => _PlaceholderPhoto(lieu['categorie']),
                        )
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        const Icon(Icons.location_on_rounded, size: 11, color: Color(0xFF0C5C6C)),
                    ...
                    if (especes.isNotEmpty)
                      Text(
                        especes.take(4).map(_especeEmoji).join(' '),
          ...
          Expanded(
            ...
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Container(height: 12, color: Colors.grey.shade200, width: double.infinity),
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.explore_off_outlined, size: 64, color: Colors.grey.shade400),
