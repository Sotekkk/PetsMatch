  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (finaliseNow && _foundUser?['uid'] != null) {
        ...
        // qui l'a toujours fait ; cette page (cession rapide sans contrat)
        // ne l'a jamais fait, le mouvement de sortie n'apparaissait donc
        // jamais dans le registre entrée/sortie légal.
        try {
          await _supa.from('registre_mouvements').insert({
            'animal_id':            widget.animal['id'],
            'uid_eleveur':          widget.uid,
            if (cedantProfileId.isNotEmpty) 'eleveur_profile_id': cedantProfileId,
            'type':                 'sortie',
            'date_mouvement':       dateCessionStr,
            'motif':                'cession',
            'destinataire_qualite': _qualite,
            'destinataire_nom':     nomComplet.isEmpty ? _nomCtrl.text.trim() : nomComplet,
            if (_adresseCtrl.text.trim().isNotEmpty) 'destinataire_adresse': _adresseCtrl.text.trim(),
          });
          if (_qualite == 'eleveur' || _qualite == 'refuge') {
            await _supa.from('registre_mouvements').insert({
              'animal_id':      widget.animal['id'],
              'uid_eleveur':    acqUid,
              ...
              'provenance_nom':     widget.nomElevage,
            });
          }
        } catch (_) {}
