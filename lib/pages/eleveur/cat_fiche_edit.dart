...
class CatFicheEdit extends StatefulWidget {
  final Map<String, dynamic> catData;

  const CatFicheEdit({Key? key, required this.catData}) : super(key: key);

  @override
  _CatFicheEditState createState() => _CatFicheEditState();
}

class _CatFicheEditState extends State<CatFicheEdit> {
  File? _image;
  ...
  Map<String, Map<String, String>> documentElevage = {
    ...
    'Test génétique du chat': {'name': 'Aucun document', 'path': ''},
  ...
  void initState() {
    super.initState();
    _descriptionController.text = widget.catData['description'] ?? '';
    _nameController.text = widget.catData['name'] ?? '';
    _colorController.text = widget.catData['color'] ?? '';
    _fatherNameController.text = widget.catData['fatherName'] ?? '';
    _motherNameController.text = widget.catData['motherName'] ?? '';
    _fatherDNAController.text = widget.catData['fatherDNA'] ?? '';
    _motherDNAController.text = widget.catData['motherDNA'] ?? '';
    sexAnimal = widget.catData['sex'] ?? 'Aucun';
    raceAnimal = widget.catData['race'] ?? 'Aucun';

    _chipNumberController.text = widget.catData['chipNumber'] ?? '';
    _coatTypeController.text = widget.catData['coatType'] ?? '';
    _birthWeightController.text = widget.catData['birthWeight'] ?? '';
    controllerDateNaissanceCat.text = widget.catData['dateOfBirth'] ?? '';
    _controller.text = widget.catData['breeding'] ?? '';
    _viensDeVotreElevage = widget.catData['breeding'] == '';
    _breedController.text = widget.catData['race'] ?? '';
    vaccines = widget.catData['vaccines'] != null
        ? List<Map<String, dynamic>>.from(widget.catData['vaccines'])
        : [];
    ...
    vermifuges = widget.catData['vermifuges'] != null
        ? List<Map<String, dynamic>>.from(widget.catData['vermifuges'])
        : [];

    chaleurs = widget.catData['chaleurs'] != null
        ? List<DateTime>.from((widget.catData['chaleurs'] as List<dynamic>)
            .map((date) => DateTime.parse(date)))
        : [];

    saillies = widget.catData['saillies'] != null
        ? List<Map<String, dynamic>>.from(widget.catData['saillies'])
        : [];
    _maleNameControllers =
        List<TextEditingController>.from(saillies.map((saillie) {
      return TextEditingController(text: saillie['maleName'] ?? '');
    }));
    if (widget.catData['documents'] is Map) {
      (widget.catData['documents'] as Map<String, dynamic>)
          .forEach((key, value) {
        if (value is String) {
    ...
    if (widget.catData['profilePicture'] != 'Aucune photo de profil') {
      _profilePictureUrl = widget.catData['profilePicture'];
    }
  ...
  Future<List<String>> loadCat() async {
    final String response =
        await rootBundle.loadString('assets/cat_breeds.json');
  ...
  Future<void> _saveData() async {
    ...
    try {
      ...
      Map<String, dynamic> catData = {
        ...
        'profilePicture':
            widget.catData['profilePicture'] ?? 'Aucune photo de profil',
        'documents': widget.catData['documents'] ?? {},
        'vaccines': vaccines,
      ...
      for (String category in documentElevage.keys) {
        if (documentElevage[category]!['path']!.isNotEmpty) {
          ...
          catData['documents'][category] =
              widget.catData['documents'][category] ?? 'Aucun document';
        }
      }

      await docRef.collection('entries').doc(docId).update(catData);
  ...
  void _addSaillie() {
    setState(() {
      saillies
          .add({'maleName': '', 'date': null}); // Ajouter une nouvelle saillie
  ...
  void _removeSaillie(int index) {
    setState(() {
      if (index < saillies.length) {
        saillies.removeAt(index); // Supprimer la saillie
        _maleNameControllers[index].dispose(); // Libérer le contrôleur
        _maleNameControllers.removeAt(index); // Supprimer le contrôleur associé
      }
    });
  }
