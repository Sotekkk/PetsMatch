...
| A38 | Profil pro — masquer le feed annonces (achat/adoption) et la section annonces éleveur. Un pro n'est pas acheteur. Garder uniquement : messagerie, services, agenda RDV, profil. Si accès direct à une URL annonce, afficher message "Réservé aux particuliers et éleveurs". | Moyenne | App + Web | `eleveur_nav.dart`, `pro_agenda.dart`, `src/app/` middleware ou layout |
...
| A42 | Messagerie — supprimer une conversation complète (avec confirmation) : suppression de tous les messages Firestore de la conversation + la conversation elle-même, visible pour l'auteur uniquement (soft delete ou suppression réelle). | Haute | App + Web | `chatScreen.dart`, `conversations_list.dart`, `src/app/messagerie/page.tsx` |
| A43 | Courbe de poids portée — graphique comparatif : afficher sur un même graphique les courbes de poids de tous les bébés d'une portée (une courbe par animal, couleur différente), avec l'âge en jours/semaines sur l'axe X. Option : superposer une autre portée pour comparaison. | Haute | App + Web | `portee_form_page.dart`, `mes-animaux/portee/page.tsx`, `animal_fiche.dart` |
| A44 | Fiche animal — pedigree : ajouter l'option photo (appareil photo + galerie) en plus du PDF existant pour uploader le document pedigree. Même crop/upload que les photos animaux. | Haute | App | `animal_fiche.dart` (section pedigree, `_pickPedigreeFile`) |
| A45 | Alerte mise en retraite femelles — notification push + badge dans la fiche animal quand la femelle approche de l'âge de retraite reproductive selon l'espèce (ex. chienne : 7 ans, chatte : 8 ans), uniquement si non stérilisée. Cloud Function quotidienne similaire à `sendChaleursNotifications`. | Moyenne | Firebase Functions + App + Web | `functions/chaleurs.js` ou nouveau fichier, `animal_fiche.dart` |
| A46 | Profils employés d'élevage — un éleveur peut créer des comptes employés rattachés à son élevage : accès aux fiches animaux (lecture + modification), planning des tâches à faire par animal/date (alimentation, soins, pesée, nettoyage), géré et révocable par l'éleveur. Table Supabase `employes` (`uid_employe`, `uid_eleveur`, `actif`) + `taches_elevage` (`titre`, `animal_id`, `date`, `statut`, `assigné_a`). | Haute | App + Web | Créer `lib/pages/eleveur/employes/`, `src/app/elevage/employes/`, table `employes` + `taches_elevage` Supabase |
