...
| ~~PT11~~ | ~~**[V2]** Lecteur puce RFID — `ChipScannerService`, 3 contextes : élevage → fiche animal, formulaire → pré-remplissage, inconnu → propose créer/déclarer~~ | ~~Haute~~ | ~~App~~ | ✅ Terminé 2026-06-03 — NETUM DJ130 mode HID clavier. `HardwareKeyboard.instance.addHandler()` capture tous keystrokes sans TextField (plus de barre Android, premier chiffre capturé). Bouton AppBar `mes_animaux.dart`, bouton scan champ puce `animal_trouve_form_page.dart` + `alerte_perdu_form_page.dart`. Résultats : fiche animal directe ou bottom sheet perdu/trouvé avec navigation vers formulaires. |
...
| ~~PLN02b~~ | ~~Page Agenda Web `/elevage/agenda` — vue semaine, routines groupées par etape_id + modal par-animal avec toggle immédiat, tâches manuelles~~ | ~~Haute~~ | ~~Web~~ | ✅ Terminé 2026-06-16 — `website/src/app/elevage/agenda/page.tsx`. Renommage "Protocoles" → "Routines" dans toute l'UI planning + Header + EleveurDashboard |
...
| ~~PLN03b~~ | ~~Agenda (menu bas) — vue groupée protocoles avec bottom sheet par-animal + dedup via etape_id~~ | ~~Haute~~ | ~~App~~ | ✅ Terminé 2026-06-16 — `agenda_page.dart` : `_buildDayTasksSection()` groupe par `etape_id`, carte compacte avec progression, `_AgendaProtoSheet` DraggableScrollable avec `CheckboxListTile` par animal + toggle immédiat `plan_taches.statut`. Validation = sync automatique (même table). |
